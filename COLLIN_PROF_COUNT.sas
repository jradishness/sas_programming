...
DATA RELEVENROLL;		*COMBINE SPRING/FALL ENROLLMENTS;
	SET ARCHIVE;
RUN;
PROC SORT DATA=RELEVENROLL;		*RESORT THE NEWLY-COMBINED DATASET;
	BY STRM INSTR_EMPLID;		
RUN;
DATA PROF_ENROLL;
...
DATA PROF_ENROLL;
	...
	WHERE INSTR_EMPLID ~=" ";
	EMPLID = INSTR_EMPLID;
	NAME = CATX(' ', INSTR_FIRST_NAME, INSTR_LAST_NAME);
...
data F_DENTON_PROF;
	set PROF_ENROLL;
	by EMPLID;
	if First.EMPLID then DENTON_TOT = 0;
...
data F_INTERNET_PROF;
	set PROF_ENROLL;
	by EMPLID;
	if First.EMPLID then INTERNET_TOT = 0;
...
data S_DENTON_PROF;
	set PROF_ENROLL;
	by EMPLID;
	if First.EMPLID then SDENTON_TOT = 0;
...
data S_COLLIN_PROF;
	set PROF_ENROLL;
	by EMPLID;
	if First.EMPLID then SCOLLIN_TOT = 0;
...
data S_INTERNET_PROF;
	set PROF_ENROLL;
	by EMPLID;
	if First.EMPLID then SINTERNET_TOT = 0;
...
DATA ACAD_GROUPS;
	SET PROF;
	KEEP ACAD_GROUP EMPLID;
RUN;
PROC TRANSPOSE DATA=ACAD_GROUPS 
			   OUT=ACAD_GROUPS_TRANS
			   PREFIX=Subject;
	VAR ACAD_GROUP;
	BY EMPLID;
RUN;
DATA FINAL_ACAD;
	SET ACAD_GROUPS_TRANS;
	DROP _NAME_ _LABEL_;
RUN;

DATA FINAL_PROF;
	MERGE PROF F_COLLIN_PROF F_DENTON_PROF F_INTERNET_PROF S_COLLIN_PROF S_DENTON_PROF S_INTERNET_PROF FINAL_ACAD;
	BY EMPLID;
	DROP ACAD_GROUP COURSE LOCATION F_CC_SECS F_DT_SECS F_OL_SECS S_CC_SECS S_DT_SECS S_OL_SECS;	*DROP EXTRANEOUS COUNTERS NOW THAT WE HAVE TOTALS;
RUN;
...
ods tagsets.excelxp file="C:\Users\jrk0200\UNT System\Clark, Allen - FriscoEnrollment\Reports\2019_PROF_COUNT.xml" style=XLsansprinter
	options(embed_titles_once='yes' autofit_height='yes' sheet_interval='none'
			sheet_name='Professor Report' absolute_column_width='8,22,8,8,8,8,8,8,8,8,8');

*REPORT THE DATA;
PROC REPORT DATA=FINAL_PROF;
	COLUMN ('Collin Professor' EMPLID NAME) 
	('Course Count'('Fall 2019' COLLIN_TOT DENTON_TOT INTERNET_TOT) 
	('Spring 2019' SCOLLIN_TOT SDENTON_TOT SINTERNET_TOT))
	('Subjects taught' SUBJECT1 SUBJECT2 SUBJECT3);
	...
	DEFINE SUBJECT1 / DISPLAY;
	DEFINE SUBJECT2 / DISPLAY;
	DEFINE SUBJECT3 / DISPLAY;
