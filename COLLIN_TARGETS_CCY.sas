...
%let DAIR_DATA="S:\UIS\Shared\sas_data_projects\enrollment_records\enrollement_dw_2020z_%sysfunc(today(),mmddyy6.)";
%LET SIMST = "S:\UIS\Shared\sas_data_projects\simsterm_records\s2020z_%sysfunc(today(),mmddyy6.)";
...
* STEPS TO COMBINE WEEKS WITHOUT DUPLICATES;      *EVERY WEEK - ADD NEW DATA STEP;
DATA REG_W1;
...
RUN;
DATA REG_W3;
	MERGE WEEK3 WEEK4 (IN=A);
	BY EMPLID;
	IF A = 0;
	DROPPED = 3;
RUN;
DATA REG_W4;
	MERGE WEEK4 WEEK5 (IN=A);
	BY EMPLID;
	IF A = 0;
	DROPPED = 4;
RUN;
DATA REG_W5;	
	MERGE WEEK5 WEEK6 (IN=A); 
	BY EMPLID;
	IF A = 0;
	DROPPED = 5; 
RUN;
DATA REG_W6;	
	MERGE WEEK6 WEEK7 (IN=A); 
	BY EMPLID;
	IF A = 0;
	DROPPED = 6; 
RUN;
DATA REG_W7;	
	MERGE WEEK7 WEEK8 (IN=A); 
	BY EMPLID;
	IF A = 0;
	DROPPED = 7; 
RUN;
DATA REG_W8;	
	MERGE WEEK8 WEEK9 (IN=A); 
	BY EMPLID;
	IF A = 0;
	DROPPED = 8; 
RUN;
DATA REG_W9;	* <- INCREMENT THIS LINE;
	MERGE WEEK9 WEEK10 (IN=A); * <- INCREMENT THIS LINE;
	BY EMPLID;
	IF A = 0;
	DROPPED = 9; * <- INCREMENT THIS LINE;
RUN;

DATA IPT;
	SET WEEK10 REG_W9 REG_W8 REG_W7 REG_W6 REG_W5 REG_W4 REG_W3 REG_W2 REG_W1;   *EVERY WEEK - UPDATE WITH NEW DATASET;
RUN;
