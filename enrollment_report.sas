...
**                                                             **
**     Every Week                                              **
**  1) add the week's enrollment to the trajectory calc        **
**  2) change old_enroll to closest archive date               **
**                                                             **
**     Every Semester                                          **
...
%LET OLD_ENROLL_DATE=051319;
%let old_enroll="S:\UIS\Shared\SAS Data Projects\enrollment_records\enrollement_dw_2019z_&OLD_enroll_DATE"; *LAST YEAR'S FILE;
...
*%let SIMSTERM="S:\UIS\Shared\SAS Data Projects\simsterm_records\s2020z_041520";
...
*%let new_enroll="S:\UIS\Shared\SAS Data Projects\enrollment_records\enrollement_dw_2020z_041520"; *THIS YEAR'S FILE;
...
**-------------------------------------------------------------**;
%LET val = %SYSFUNC(INPUTN(&old_enroll_date, MMDDYY6.));
%LET OLD_DATE_FORM = %SYSFUNC(PUTN(&VAL, WORDDATE.));
...
DATA EX_REPORT;		*ALL FOUR TIME PERIODS MERGED;
	MERGE FEX1 FEX2 FEX3 FEX4 FEX5;
	BY GROUP;
RUN;
...
%SEMESTER(APR_B_19, "S:\UIS\Shared\SAS Data Projects\enrollment_records\enrollement_dw_2020z_041320", APR_B, 'D-Mid April', 2020)
...
DATA CTRAJECTORY_18;				* CHEC - COMBINE EACH TOTAL FOR THE PREVIOUS YEAR;
	SET CSEP_A_18 CAPR_A_18 CAPR_B_18 CMAY_A_18 CMAY_B_18 CJUN_A_18 CJUN_B_18 CJUL_A_18 CJUL_B_18 CAUG_A_18 CAUG_B_18 CSEP_B_18;
	C18=TOTAL;
	KEEP SEM C18;
RUN;
DATA CTRAJECTORY_19;			* CHEC - COMBINE EACH TOTAL FOR THIS YEAR;
	SET CSEP_A_19 CMAR_A_19 CMAR_B_19 CAPR_A_19 CAPR_B_19 CMAY_A_19 CMAY_B_19 CJUN_A_19 CJUN_B_19 CJUL_A_19 CJUL_B_19 CAUG_A_19 CAUG_B_19 CSEP_B_19;
	C19=TOTAL;
	KEEP SEM  C19;
RUN;
...
DATA FTRAJECTORY_18;				*Frisco - COMBINE EACH TOTAL FOR THE PREVIOUS YEAR;
	SET FSEP_A_18 FAPR_A_18 FAPR_B_18 FMAY_A_18 FMAY_B_18 FJUN_A_18 FJUN_B_18 FJUL_A_18 FJUL_B_18 FAUG_A_18 FAUG_B_18 FSEP_B_18;
	F18=TOTAL;
	KEEP SEM F18;
RUN;

DATA FTRAJECTORY_19;			* Frisco - COMBINE EACH TOTAL FOR THIS YEAR;
	SET FSEP_A_19 FMAR_A_19 FMAR_B_19 FAPR_A_19 FAPR_B_19 FMAY_A_19 FMAY_B_19 FJUN_A_19 FJUN_B_19 FJUL_A_19 FJUL_B_19 FAUG_A_19 FAUG_B_19 FSEP_B_19;
	F19=TOTAL;
	KEEP SEM F19;
RUN;
...
RUN;
******* LAST_YEAR;
DATA UGRD_LAST;		*GATHERING ARCHIVED ENROLLMENT NUMBERS FOR SPRING;
	SET &old_enroll;
	WHERE LOCATION IN ('FRSC', 'Z-INSPK') AND CRSE_CAREER='UGRD';
	GROUPING='1-UGRD';
RUN;
PROC SORT DATA=UGRD_LAST OUT=UGENRL_LAST;		*COUNTING THE ENROLLMENT;
	BY ACAD_GROUP;
RUN;
DATA UGENRL_LAST;						*COUNTING THE ENROLLMENT;
	SET UGENRL_LAST;
	BY ACAD_GROUP;
	IF LAST.ACAD_GROUP;                 /*Go back if not last.id     */
  	LAST_YEAR=_n_-sum(lag(_n_),0);  /*At last.id, calculate COUNT*/
	KEEP ACAD_GROUP LAST_YEAR;
RUN;
***FINAL;
DATA UGFINAL;
	MERGE UGENRL(IN=A) UGSEC UGENRL_FALL UGENRL_SPR UGENRL_LAST;
	BY ACAD_GROUP;
	IF A=1;
	DROP COURSE;
RUN;
...
DATA GRENRL_SPR;						*COUNTING THE ENROLLMENT;
	...
	KEEP ACAD_GROUP SPRING;
RUN;
DATA GRAD_LAST;		*GATHERING ARCHIVED ENROLLMENT NUMBERS FOR SPRING;
	SET &OLD_ENROLL;
	WHERE LOCATION IN ('FRSC', 'Z-INSPK') AND CRSE_CAREER='GRAD';
	GROUPING='2-GRAD';
RUN;
PROC SORT DATA=GRAD_LAST OUT=GRENRL_LAST;		*COUNTING THE ENROLLMENT;
	BY ACAD_GROUP;
RUN;
DATA GRENRL_LAST;						*COUNTING THE ENROLLMENT;
	SET GRENRL_LAST;
	BY ACAD_GROUP;
	IF LAST.ACAD_GROUP;                 /*Go back if not last.id     */
  	LAST_YEAR=_n_-sum(lag(_n_),0);  /*At last.id, calculate COUNT*/
...
RUN;
DATA GRFINAL;
	MERGE GRENRL(IN=A) GRSEC GRENRL_FALL GRENRL_SPR GRENRL_LAST;
	BY ACAD_GROUP;
	IF A=1;
	DROP COURSE;
RUN;
...
DATA CHENRL_SPR;						*COUNTING THE ENROLLMENT;
	...
	KEEP ACAD_GROUP SPRING;
RUN;
DATA CHEC_LAST;		*GATHERING ARCHIVED ENROLLMENT NUMBERS FOR SPRING;
	SET &OLD_ENROLL;
	WHERE LOCATION IN ('Z-CHEC');
	GROUPING='3-CHEC';
RUN;
PROC SORT DATA=CHEC_LAST OUT=CHENRL_LAST;		*COUNTING THE ENROLLMENT;
	BY ACAD_GROUP;
RUN;
DATA CHENRL_LAST;						*COUNTING THE ENROLLMENT;
	SET CHENRL_LAST;
	BY ACAD_GROUP;
	IF LAST.ACAD_GROUP;                 /*Go back if not last.id     */
  	LAST_YEAR=_n_-sum(lag(_n_),0);  /*At last.id, calculate COUNT*/
...
RUN;
DATA CHFINAL;
	MERGE CHENRL(IN=A) CHSEC CHENRL_FALL CHENRL_SPR CHENRL_LAST;
	BY ACAD_GROUP;
	IF A=1;
	DROP COURSE;
RUN;
...
DATA BAENRL_SPR;						*COUNTING THE ENROLLMENT;
	...
	KEEP ACAD_GROUP SPRING;
RUN;
DATA BAAS_LAST;		*GATHERING ARCHIVED ENROLLMENT NUMBERS FOR SPRING;
	SET &OLD_ENROLL;
	WHERE SUBJECT='BAAS';
	GROUPING='4-BAAS';
RUN;
PROC SORT DATA=BAAS_LAST OUT=BAENRL_LAST;		*COUNTING THE ENROLLMENT;
	BY ACAD_GROUP;
RUN;
DATA BAENRL_LAST;						*COUNTING THE ENROLLMENT;
	SET BAENRL_LAST;
	BY ACAD_GROUP;
	IF LAST.ACAD_GROUP;                 /*Go back if not last.id     */
  	LAST_YEAR=_n_-sum(lag(_n_),0);  /*At last.id, calculate COUNT*/
...
RUN;
DATA BAFINAL;
	MERGE BAENRL(IN=A) BASEC BAENRL_FALL BAENRL_SPR BAENRL_LAST;
	BY ACAD_GROUP;
	IF A=1;
	DROP COURSE;
RUN;
...
PROC REPORT DATA=EX_REPORT;
	COLUMN GROUP ('Executive Enrollment Report' TODAY LAST_YEAR SPRING FALL
				 YEAR_CHANGE SEM_CHANGE) TYPE;
	DEFINE GROUP / DISPLAY 'Campus';
	DEFINE TODAY / ANALYSIS "Enrollment as of %sysfunc(today(), WORDDATE.)";
	DEFINE LAST_YEAR / ANALYSIS 'Enrollment Same Day Last Year';
	DEFINE SPRING / ANALYSIS 'Final Enrollment Spring 2020';
	DEFINE FALL / ANALYSIS 'Final Enrollment Fall 2019';
	DEFINE YEAR_CHANGE / COMPUTED 'Over/Under Fall 2019 ' FORMAT=PERCENTN8.1;
	DEFINE SEM_CHANGE / COMPUTED 'Over/Under Spring 2020' FORMAT=PERCENTN8.1;
...
PROC REPORT DATA=CAPS;
	COLUMN ('HP-IP Classroom Space Use' ENRL_TOT('Capacities' ROOM_CAP ENRL_CAP) ('Use Percentage' POT_USE CUR_USE)) ;
...
ods tagsets.excelxp options(embedded_titles='yes' autofit_height='yes' sheet_interval='none'
			sheet_NAME='College Report' absolute_column_width='20,8,8,8,15,15,8,8,8');
PROC REPORT DATA=COLLEGE_FINAL;
	BY GROUPING;
	COLUMN A_GRP_DESCR SECTIONS ('Enrollment' FALL SPRING TODAY)('Day In Time' LAST_YEAR LAST_OU)('Growth Since' FCHANGE SCHANGE);
	...
	DEFINE LAST_YEAR / ANALYSIS "#'s as of &OLD_DATE_FORM";
	DEFINE LAST_OU / COMPUTED 'Over-under' FORMAT=PERCENTN8.1;
	DEFINE FCHANGE / COMPUTED 'Fall 2019' FORMAT=PERCENTN8.1;
	DEFINE SCHANGE / COMPUTED 'Spring 2020' FORMAT=PERCENTN8.1;
	COMPUTE LAST_OU;
		LAST_OU = (TODAY.SUM - LAST_YEAR.SUM) / LAST_YEAR.SUM;
	ENDCOMP;
	COMPUTE FCHANGE;
...
PROC SGPLOT DATA = TRAJECTORY;
 ...
 MARKERS LINEATTRS = (THICKNESS = 2);
	...
	YAXIS LABEL = 'Enrollment Total' GRID VALUES = (0 TO 3000 BY 100);
